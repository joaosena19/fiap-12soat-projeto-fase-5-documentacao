...
Tenho afinidade com SQL e já usava PostgreSQL com Entity Framework Core nas fases anteriores. O PostgreSQL funciona muito bem com .NET via Npgsql e tem suporte nativo a JSONB, que é útil para armazenar campos de análise de IA com estrutura variável (ver [ADR 0006](0006_adr_campos_mapeados_como_jsonb.md)).
...
Também avaliei o DynamoDB, que é serverless e tem custo atrativo no free tier. Contudo, o modelo de acesso (key-value) adicionaria complexidade desnecessária para consultas que são naturalmente relacionais.

Optei por usar o mesmo engine (PostgreSQL) para todos os três serviços. Usar o mesmo banco simplifica a operação — uma única expertise, um único ORM, uma única configuração de Terraform para RDS.
...
* Três instâncias RDS para gerenciar (custo e monitoramento).
