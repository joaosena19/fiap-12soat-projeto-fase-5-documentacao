...
O processamento é disparado por mensagem, não por requisição HTTP. A análise via LLM leva segundos — não cabe num request/response síncrono. Se fosse uma API, o usuário teria que aguardar uma resposta síncrona de uma operação lenta, o que é uma experiência ruim.
...
* Não é possível consultar o Processamento diretamente. O status é consultado via Relatório (Veja [ADR 0014 - Relatório como Hub de Agregação de Eventos](0014_adr_relatorio_como_hub_de_agregacao.md)).
