...
Usuários poderiam enviar o mesmo arquivo mais de uma vez. Era necessário decidir como tratar duplicatas para evitar desperdício de processamento pela LLM.
...
Se o hash já existir e o processamento anterior tiver concluído com sucesso, o sistema retorna HTTP 200 com o resultado existente — sem guardar upload extra e sem consumir LLM. Se o processamento anterior tiver falhado, o sistema republica a mensagem para reprocessamento (Veja [ADR 0021 - Retry como Re-upload](0021_adr_retry_como_re_upload.md)).

SHA-256 é resistente a colisões e rápido para arquivos de até 5MB (Veja [ADR 0016 - Limite de 5MB por Arquivo](0016_adr_limite_de_5mb_por_arquivo.md)). O overhead de cálculo é insignificante comparado ao custo de uma chamada à LLM.
